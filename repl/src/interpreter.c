...
static inline int is_int(value_t *node) { return (node->valueType == VAR_INT); }
...
// Converts a type and value into a value_t
// TODO: Closure
...
// Looks up a value corresponding to a name
// Will recurse to the parent if it is not found in the current scope
value_t *env_lookup(env_t *env, char *name, int nameLen) {
  ...
  while (binding != NULL) {
    if (strncmp(binding->name, name, nameLen) == 0 &&
        binding->name[nameLen] == '\0')
...
value_t *evaluate_tc(ast_node_t *node, env_t *env, int in_tailcall) {
  ...
  case NODE_STR:
    ...
    if (create_binding(env, node->token.str, node->token.len, left) !=
        MU_SUCCESS) {
      value_release(left);
      return NULL;
    }
    ...
    // Trampoline loop — keeps bouncing until we get a real value
    for (;;) {
      ...
      // All params bound — evaluate the body
      // If this is a tailcall function, tell evaluate_tc
      // so nested NODE_APPLY returns thunks instead of recursing
      ...
      // If we got a thunk back, bounce — rebind fn/arg and loop
